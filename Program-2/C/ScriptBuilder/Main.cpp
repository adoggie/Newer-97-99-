...
//------------------------------------------------------------
void TFormMain::RidChar(AnsiString &str,char c)  // 删除指定字符
{
    AnsiString s;
                ...
                while(!QryTmpRecord->Eof)
                {
                    for(int i=0;i<lstTmpColumn->Count;i++)
                    {
                        ...
                        else if(strType=="DATE")
                        {
                            ...
                            {
                                TDateTime  dt(Value);
                                Value=dt.FormatString( "MM-DD-YYYY HH:NN:SS");
                                Value="TO_DATE('"+Value+"','MM-DD-YYYY HH24:MI:SS')";
                            }
                        }
                    ...
                    //RidChar(strSQL,(char)'\r');  //删除指定字符，避免RICHEDIT出错
                    //RidChar(strSQL,(char)0xA);  //删除指定字符，避免RICHEDIT出错
                    Memo1->Lines->Add(strSQL);
                    strSQL=strStore;
                    QryTmpRecord->Next();
                }
...
void __fastcall TFormMain::btnExecuteClick(TObject *Sender)
{
    QryData->DatabaseName= DBConnect->DatabaseName;
    QryData->Close();
	QryData->SQL=edtSql->Lines;
	QryData->SQL->Text=QryData->SQL->Text.Trim().UpperCase();
    if(QryData->SQL->Text=="")	return;
	if(QryData->SQL->Text.AnsiPos("SELECT")!=0)
    QryData->Open();
	else
    QryData->ExecSQL();
